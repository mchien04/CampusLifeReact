...
| :--- | :--- | :--- | :--- |
| **Phase 1** | Đồng bộ hóa Types và Cập nhật API Services | ✅ Hoàn thành | - Định nghĩa các Enum và Interface cho Luật điểm động (`scoreRules`).<br>- Cập nhật API `eventAPI.ts` để nhận raw array `ActivityResponse[]`.<br>- Sửa lỗi upload ảnh ở `uploadAPI.ts` lấy trực tiếp URL dạng string.<br>- Cập nhật model nộp bài tập với danh sách `attachments`. |
| **Phase 2** | Luật Tính Điểm Hoạt Động (Event Score Rules) & UI | ✅ Hoàn thành | - Phát triển component cấu hình luật điểm động `ScoreRulesForm.tsx`.<br>- Tích hợp form luật điểm vào giao diện Admin tạo/sửa hoạt động.<br>- Tạo component `ScoreRulesDisplay.tsx` hiển thị danh sách luật điểm dưới dạng badge trực quan.<br>- Loại bỏ các input điểm tĩnh cũ (`maxPoints`, `penaltyPointsIncomplete`). |
| **Phase 3** | Xem Lịch Sử Điểm Theo Ledger (Score Ledger) | ✅ Hoàn thành | - Cập nhật `ViewScores.tsx` và `ManagerScores.tsx` để lấy lịch sử điểm chính xác từ bảng Ledger của sinh viên.<br>- Map chi tiết nguồn điểm (`ACTIVITY_PARTICIPATION`, `MINIGAME_ATTEMPT`,...) sang nhãn tiếng Việt trực quan kèm badge màu sắc. |
| **Phase 4** | Nộp Bài Tập Hỗ Trợ Attachment & Upload chuẩn | ✅ Hoàn thành | - Nâng cấp form nộp bài tập ở `StudentTasks.tsx` cho phép chọn riêng biệt `files` và `images` gửi qua `FormData`.<br>- Cập nhật modal xem chi tiết bài nộp của Manager hỗ trợ preview hình ảnh (lightbox) và tải về tệp tin.<br>- Đồng bộ hóa trạng thái bài nộp qua trường `isCompleted`. |
| **Phase 5** | Cải Tiến Luồng Chơi & Nộp Bài Minigame | ✅ Hoàn thành | - Đổi cấu trúc nộp đáp án minigame sang dạng Map `Record<string, number>`.<br>- Nhận diện kết quả đỗ/trượt qua trạng thái `status === "PASSED"`.<br>- Hiển thị số điểm nhận được thực tế từ score engine (`pointsEarned`).<br>- Thêm cờ `showAnswers` vào các Form quản lý quiz để Backend cho phép/chặn xem đáp án. |
| **Phase 6** | Màn hình Admin Tools & Dashboard | ✅ Hoàn thành | - Tạo màn hình `AdminTools.tsx`.<br>- Thêm chức năng **Kiểm tra Tasks quá hạn** gọi API `/api/admin/system/trigger-check-overdue`.<br>- Thêm chức năng **Dọn dẹp dữ liệu rác** gọi API `/api/admin/system/cleanup-orphan-data`.<br>- Tích hợp Admin Tools vào thanh Sidebar của `ManagerLayout.tsx` và App Route. |
| **Cleanup** | Loại bỏ hoàn toàn Legacy Score Fields | ✅ Hoàn thành | - Rà soát toàn bộ dự án, xóa bỏ hoàn toàn tất cả thuộc tính điểm tĩnh cũ (`maxPoints`, `penaltyPointsIncomplete`, `rewardPoints`) trên cả Models, UI, State và Props.<br>- Đảm bảo toàn bộ dự án biên dịch thành công 100% không còn lỗi TypeScript. |
...
- **Đồng bộ hóa Types**:
  ...
  - Tại [submission.ts](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/types/submission.ts), bổ sung type `SubmissionAttachment` để lưu danh sách file đính kèm đã phân loại cụ thể dạng `file` hoặc `image`.
...
- **Sửa đổi API Client**:
  - Tại [eventAPI.ts](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/services/eventAPI.ts), cập nhật các hàm filter sự kiện (`getEventsByScoreType`, `getEventsByDepartment`, `getMyEvents`, `getEventsByMonth`) trả về thẳng mảng `ActivityResponse[]` (không bọc qua wrapper `.body` hoặc `.data` lồng) để tương thích với Backend API mới.
...
- **Giao diện Quản trị viên (Chấm điểm)**:
  - Cập nhật [TaskSubmissions.tsx](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/pages/TaskSubmissions.tsx) và form quản lý bài tập để hiển thị đúng thông tin của bài nộp dạng `attachments[]` thay cho `fileUrls` cũ.
...
- Cập nhật thêm thuộc tính cấu hình `showAnswers` ở [QuizForm.tsx](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/components/minigame/QuizForm.tsx) để xác định việc sinh viên có được phép xem kết quả chi tiết hay không.

### Phase 6: Màn hình Admin Tools & Dashboard
- **Tạo giao diện công cụ Quản trị viên**:
  - Tạo mới trang [AdminTools.tsx](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/pages/admin/AdminTools.tsx) cho phép Quản trị viên thực hiện các tác vụ dọn dẹp và bảo trì hệ thống.
  - Tích hợp 2 API quan trọng vào [adminAPI.ts](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/services/adminAPI.ts): `triggerCheckOverdue` và `cleanupOrphanData`.
  - Cập nhật [ManagerLayout.tsx](file:///d:/2025-2026%20HKI/TLCN/campuslifereact/src/components/layout/ManagerLayout.tsx) để thêm mục "Công cụ hệ thống" dành riêng cho role ADMIN.

### Phase 7: Xoá Bỏ Hoàn Toàn Các Trường Điểm Legacy (Dọn Dẹp & Tối Ưu)
...
- **Dọn dẹp Giao diện & Component**: Loại bỏ hoàn toàn các trường nhập liệu điểm tĩnh cũ trong các form và card:
  ...
  - Và các component quản lý danh sách hoạt động, sự kiện khác.
- **Sửa lỗi Incident**:
...
- **Kiểm tra Biên dịch (Build/Compile)**:
  ...
  - Kết quả: **Dự án hoàn toàn sạch lỗi biên dịch (0 TypeScript Errors)**, tất cả các component đều đảm bảo type-safe tuyệt đối với hợp đồng API mới.
